...
# Create table for users (Added branch, batch, course columns to support deep profile autofills)
cursor.execute("""
...
# Clean up any legacy single static student account to prevent conflicts with our master test matrix
cursor.execute("DELETE FROM users WHERE email='student@ccet.edu'")

# Insert sample data with all roles (Enhanced to include specific credentials for multi-branch autofill testing)
users_data = [
    ...
    # Master Test Pool: Accounts mapped directly to cross-platform possibilities
...
# ✅ table for messages (persistent stacking)
cursor.execute("""
...
# ✅ table for syllabus (Updated to store explicit metadata metrics for clean execution context)
cursor.execute("""
...
# Wipe syllabus table prior to loading our massive multi-branch matrix to prevent stale data collations
cursor.execute("DELETE FROM syllabus")

# ✅ Seeding complete syllabus curriculum maps for all 8 semesters across all departments
cursor.executescript("""
-- 1. COMPUTER SCIENCE & ENGINEERING (CSE) - COMPLETE REAL CURRICULUM SCHEMA
...
-- 2. ELECTRONICS & COMMUNICATION ENGINEERING (ECE) - 8 SEMESTER MATRIX
INSERT INTO syllabus (branch, semester, subject, subject_type, credits) VALUES
...
-- 3. CIVIL ENGINEERING - 8 SEMESTER MATRIX
INSERT INTO syllabus (branch, semester, subject, subject_type, credits) VALUES
...
-- 4. MECHANICAL ENGINEERING (MECH) - 8 SEMESTER MATRIX
INSERT INTO syllabus (branch, semester, subject, subject_type, credits) VALUES
...
print("Database architecture optimized with complete 8-semester data models for all possibilities!")